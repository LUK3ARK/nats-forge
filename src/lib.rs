         ...
         for server in &self.config.servers {
             ...
             for account in &server.accounts {
                 ...
                 for user in &account.users {
                     let creds_path = create_user(account, user, &server.output_dir, &self.store_dir.path().to_path_buf()).await?;
         ...
         for server in &self.config.servers {
             ...
             for remote in &server.leafnodes.remotes {
                 if let Some((source_path, source_dir)) = creds_map.get(&remote.credentials) {
                     ...
                     std::fs::copy(&abs_source, &abs_dest)
                         .context(format!("Failed to copy creds from {} to {}", abs_source.display(), abs_dest.display()))?;
                 }
             }
             ...
             for (account_name, account_jwt) in &account_jwts {
                 ...
                 std::fs::write(&dest_jwt_path, account_jwt)
                     .context(format!("Failed to copy JWT for {} to {}", account_name, dest_jwt_path.display()))?;
             }
         }
